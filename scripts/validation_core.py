...
REQUIRED_FILES = [
    ...
    *CANONICAL_DOCUMENTS.keys(),
    *CANONICAL_DOCUMENTS.values(),
]
    ...
    *,
    ...
    if expected_type is not None:
        ...
        if not any(_json_type_matches(value, item) for item in allowed):
            return [
                ValidationIssue(
                    "PFV-110",
                    f"{document}{path}: expected {' or '.join(allowed)}, got {_type_name(value)}",
                )
            ]

    if "const" in schema and value != schema["const"]:
        issues.append(
            ValidationIssue("PFV-112", f"{document}{path}: value must equal {schema['const']!r}")
        )
    if "enum" in schema and value not in schema["enum"]:
        issues.append(
            ValidationIssue("PFV-112", f"{document}{path}: value is outside the allowed enum")
        )

    if isinstance(value, dict):
        required = schema.get("required", [])
        if not isinstance(required, list):
            return [ValidationIssue("PFV-103", f"{document}: invalid required declaration at {path}")]
        for key in required:
            if key not in value:
                issues.append(
                    ValidationIssue("PFV-111", f"{document}{path}: missing required property {key!r}")
                )
        properties = schema.get("properties", {})
        if not isinstance(properties, dict):
            return [ValidationIssue("PFV-103", f"{document}: invalid properties declaration at {path}")]
        ...
        for key, item in value.items():
            ...
            if key in properties:
                ...
                if not isinstance(child_schema, dict):
                    issues.append(
                        ValidationIssue("PFV-103", f"{document}: invalid child schema at {child_path}")
                    )
                else:
                    issues.extend(
                ...
                issues.append(
                    ValidationIssue("PFV-115", f"{document}{path}: unexpected property {key!r}")
                )
        ...
        if isinstance(min_properties, int) and len(value) < min_properties:
            issues.append(
                ValidationIssue(
                    "PFV-113",
                    f"{document}{path}: expected at least {min_properties} properties",
                )
            )
    ...
    if isinstance(value, list):
        ...
        if isinstance(min_items, int) and len(value) < min_items:
            issues.append(
                ValidationIssue("PFV-113", f"{document}{path}: expected at least {min_items} items")
            )
        if isinstance(max_items, int) and len(value) > max_items:
            issues.append(
                ValidationIssue("PFV-113", f"{document}{path}: expected at most {max_items} items")
            )
    ...
    if isinstance(value, str):
        ...
        if isinstance(min_length, int) and len(value) < min_length:
            issues.append(
                ValidationIssue("PFV-114", f"{document}{path}: string is shorter than {min_length}")
            )
        pattern = schema.get("pattern")
        if isinstance(pattern, str) and re.search(pattern, value) is None:
            issues.append(
                ValidationIssue("PFV-116", f"{document}{path}: string does not match required pattern")
            )
    ...
    if isinstance(value, (int, float)) and not isinstance(value, bool):
        ...
        if isinstance(minimum, (int, float)) and value < minimum:
            issues.append(
                ValidationIssue("PFV-114", f"{document}{path}: number is below minimum {minimum}")
            )
        if isinstance(maximum, (int, float)) and value > maximum:
            issues.append(
                ValidationIssue("PFV-114", f"{document}{path}: number exceeds maximum {maximum}")
            )
